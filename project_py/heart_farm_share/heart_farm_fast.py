"""
============================================================================
 Cookie Run — HEART FARM BOT  ***FAST / AGGRESSIVE edition***
============================================================================
 This is a throughput-tuned variant of heart_farm.py. It does the exact same
 thing (create disposable guests -> friend -> heart -> collect -> cleanup ->
 repeat) but deliberately pushes the parts the safe version keeps low, to run
 as fast as possible. Use heart_farm.py if you want the conservative defaults.

 What is different from the safe version (heart_farm.py):
   1. Sized HTTP connection pools -- the shared requests.Session defaulted to
      only 10 pooled connections (urllib3 default), which silently serialized
      login/initMember once WORKERS went above ~10. Now the pool is sized to
      WORKERS, so the producer actually runs as parallel as configured. This
      is a real speedup even at the OLD concurrency and carries NO extra risk.
   2. A small POOL of shared gRPC channels for guests (round-robin) instead of
      one, so a single HTTP/2 connection's concurrent-stream cap is not the
      ceiling when WORKERS is high. Also no extra account risk.
   3. Higher default concurrency knobs (WORKERS, STREAM_CONCURRENCY). THIS is
      the ban-risk lever -- see the warning on STREAM_CONCURRENCY below.

 !!! IMPORTANT / BAN RISK !!!
   STREAM_CONCURRENCY controls how many writes hit YOUR real account at once
   (accepting friends + receiving hearts). The safe version uses 2 on purpose.
   Raising it makes the run faster but the game's servers may rate-limit or
   flag the account for unusually fast friend/heart activity. There is no
   guarantee against a rate-limit or ban at high values -- YOU choose the
   trade-off. Start moderate (4-6), watch for failures, back off if you see
   lots of accept/sendlife errors. Use at your own risk.

 STANDALONE, single-file -- needs only itself + _descriptors.bin next to it.

 SETUP:  pip install -r requirements.txt   then   python heart_farm_fast.py
 See README.md for background / troubleshooting.
============================================================================
"""
...
WORKERS             = 200      # producer parallelism: how many guests are being
                                # created + friend-requested at once. Higher =
                                # faster guest supply, bounded by your proxy /
                                # the login server. No direct account risk (guests
                                # are throwaway). Try 150-300 with a good proxy.
STREAM_CONCURRENCY  = 6        # !!! BAN-RISK LEVER !!! parallel writes to YOUR
                                # OWN account (accept friend + receive heart).
                                # Safe version = 2. 4-6 = aggressive but usually
                                # fine, 8-12 = fast but higher rate-limit/flag
                                # risk. If you see many accept/sendlife failures,
...
GUEST_CHANNEL_POOL  = 6        # number of shared gRPC channels for guests (round-
                                ...
                                # high WORKERS. Pure perf, no account risk.
HTTP_POOL_SIZE      = 0        # urllib3 pool size for DS/login HTTP. 0 = auto
                                # (=WORKERS). MUST be >= WORKERS or high WORKERS
                                # silently serialize at ~10 connections.
CONFIRM_AGGRESSIVE  = True     # print the ban-risk warning and require a keypress
                                # before starting. Set False to skip the prompt.
...
def main():
    ...
    global STREAM_CONCURRENCY

    print("==========================================================")
    print("       Cookie Run - HEART FARM BOT  *FAST / AGGRESSIVE*    ")
    print("==========================================================")
    ...
    # 4. Aggression lever (writes to YOUR account -> ban risk). Runtime override.
    conc_input = input(
        "Accept/Send concurrency (BAN-RISK lever, safe=2) [Default: %d]: " % STREAM_CONCURRENCY).strip()
    if conc_input.isdigit() and int(conc_input) > 0:
        STREAM_CONCURRENCY = int(conc_input)

    print("\n[+] Starting heart farm session (FAST mode)...")
    print("  - Account: %s" % MAIN_EMAIL)
    print("  - Proxy: %s" % (PROXY_URL if PROXY_URL else "None"))
    print("  - Target Hearts: %d" % TARGET_HEARTS)
    print("  - WORKERS (producer): %d   HTTP pool: %d   guest gRPC channels: %d"
          % (WORKERS, _HTTP_POOL_N, max(1, GUEST_CHANNEL_POOL)))
    print("  - STREAM_CONCURRENCY (account writes): %d  <-- ban-risk lever" % STREAM_CONCURRENCY)
    if STREAM_CONCURRENCY > 2:
        print("  ! WARNING: concurrency > 2 sends friend/heart activity to your account")
        print("    faster than the safe default. The game may rate-limit or flag it.")
        print("    If you see many accept/sendlife failures, restart with a lower value.")
    print("==========================================================\n")

    if CONFIRM_AGGRESSIVE and STREAM_CONCURRENCY > 2 and not DRY_RUN:
        ack = input("Proceed at this aggression level? [y/N]: ").strip().lower()
        if ack not in ("y", "yes"):
            raise SystemExit("ยกเลิก -- ปรับ STREAM_CONCURRENCY ให้ต่ำลงแล้วรันใหม่ได้")
