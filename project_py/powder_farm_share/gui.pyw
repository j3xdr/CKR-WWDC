...
APP_TITLE = "Cookie Run — Magic Powder Farm"
SUBTITLE = "แปลง coin เป็นผงเวทมนตร์ (Login เช็คเงินก่อน + ยืนยันก่อนรัน)"
FLOW = [
    "1) Login DevPlay",
    "2) เช็ค Coin คงเหลือ + อัตราแลกเปลี่ยน (ระบบไม่ให้รันเกิน coin ที่มี)",
    "3) ใส่จำนวนรอบที่ต้องการ",
    "4) ยืนยันค่าใช้จ่าย + ผงที่จะได้ แล้วเริ่มซื้อ-ย่อยอัตโนมัติ",
]
...
FIELDS = [
    ...
    {"key": "runs", "label": "จำนวนรอบที่จะรัน (ห้ามเกิน coin ที่มี)", "kind": "int", "required": True},
]

# script filenames. "fast" is optional (adds a FAST toggle if present).
NORMAL_SCRIPT = "powder_farm.py"
FAST_SCRIPT = None

# sibling files that must exist before running (checked, friendly error)
REQUIRED_FILES = ["treasure.json"]


def build_stdin(v, fast):
    """Answers fed to the script's input() prompts, in order.
    powder asks: email, password, run-count, then a [y/N] confirm."""
    return [v["email"], v["password"], v["runs"], "y"]
# ===========================================================================
