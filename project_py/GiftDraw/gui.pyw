...
APP_TITLE = "Cookie Run — Gift Draw"
SUBTITLE = "เปิดกล่องขวัญฟรี 0 เพชร"
FLOW = [
    "1) Login DevPlay",
    "2) เช็คจำนวนกล่อง Gift Draw ที่มี",
    "3) เลือกจำนวนกล่องที่จะเปิด (เว้นว่าง = ทั้งหมด)",
    "4) เปิดกล่อง + สรุปของที่ได้",
]
...
FIELDS = [
    ...
    {"key": "count", "label": "จำนวนกล่อง (เว้นว่าง = เปิดทั้งหมด)", "kind": "int"},
]

# script filenames. "fast" is optional (adds a FAST toggle if present).
NORMAL_SCRIPT = "gift_draw.py"
FAST_SCRIPT = None
...
def build_stdin(v, fast):
    ...
    return [v["email"], v["password"], v.get("count", "")]
# ===========================================================================
