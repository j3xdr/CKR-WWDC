...
APP_TITLE = "Cookie Run — Party Run Farm"
SUBTITLE = "ฟาร์ม coin/exp จาก Party Run"
FLOW = [
    "1) Login DevPlay",
    "2) เลือก preset (Max Profit / Safe)",
    "3) ใส่จำนวนรอบ",
    "4) ต่อรอบ: เคลียร์ค้าง → matchmake → เล่น+ส่งผล → เคลม reward",
]
...
FIELDS = [
    ...
    {"key": "preset", "label": "Preset", "kind": "choice",
     "options": [("Max Profit (Coin 449k / EXP 52k)", "1"), ("Safe (Coin 200k / EXP 25k)", "2")]},
    {"key": "rounds", "label": "จำนวนรอบ (เว้นว่าง = 1)", "kind": "int"},
    {"key": "playtime", "label": "FAST: PLAYTIME_SECONDS (เว้นว่าง = 10 • ต่ำไป = INVALID PLAY)",
     "kind": "int", "fast_only": True},
]

# script filenames. "fast" is optional (adds a FAST toggle if present).
NORMAL_SCRIPT = "partyrun_single_file.py"
FAST_SCRIPT = "partyrun_fast.py"
...
def build_stdin(v, fast):
    ...
    normal: email, password, preset, rounds.
    fast:   ... + PLAYTIME override + a [y/N] confirm."""
    lines = [v["email"], v["password"], v.get("preset", "1"), v.get("rounds", "")]
    if fast:
        lines += [v.get("playtime", ""), "y"]
    return lines
# ===========================================================================
