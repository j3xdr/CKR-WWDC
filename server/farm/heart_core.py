"""
Heart farm core — create disposable guest accounts, have each send your account
one heart, collect, delete the guests, repeat until the target is reached.

Ported from project_py/heart_farm_share/heart_farm_fast.py for the CKR WWDC web
API. The tuned pieces of that script are kept as-is because they are pure
throughput with no account risk:

  * HTTP connection pools sized to the worker count — urllib3 defaults to 10,
    which silently serializes login/initMember once WORKERS goes above that.
  * A small round-robin pool of gRPC channels for guests, so one HTTP/2
    connection's concurrent-stream cap is not the ceiling.

What deliberately does NOT carry over is that script's aggressive defaults.
WORKERS=200 would exhaust a small server instance, and STREAM_CONCURRENCY=6
sends friend/heart writes to the user's real account fast enough to risk a
rate-limit or a flag. Both default to conservative values here and are
overridden per job by the caller, which reads admin settings.

Guest creation is also rate-limited by the game at roughly six accounts per
source IP, so a server behind a single IP needs a rotating proxy to make any
real progress. The API refuses the job when one is not configured rather than
letting a run fail halfway through.
"""

# ---------------------------------------------------------------------------
# Defaults. run_heart_job() overrides these per call; nothing here is edited
# by hand on the server the way the standalone script expects.
# ---------------------------------------------------------------------------
MAIN_EMAIL    = ""
MAIN_PASSWORD = ""
...
TARGET_HEARTS = 100            # total hearts wanted, across as many automatic
                                ...
                                # 300 minus the account's real friend count).

PROXY_URL = ""                 # rotating proxy for guest creation only; the
                                # main account never goes through it.

WORKERS             = 30       # producer parallelism (guest create + friend
                                # request). The standalone script defaults to
                                # 200; that many threads is far past what a
                                # small web instance can carry.
STREAM_CONCURRENCY  = 2        # BAN-RISK LEVER: parallel writes to the user's
                                # OWN account. 2 is the safe value and the
                                # default here regardless of what the desktop
                                # script uses.
ACCEPT_RETRIES      = 3
ACCEPT_RETRY_SLEEP  = 0.3
GUEST_CHANNEL_POOL  = 6        # shared gRPC channels for guests (round-robin);
                                # pure perf, no account risk.
HTTP_POOL_SIZE      = 0        # urllib3 pool size; 0 = auto (=WORKERS). Must be
                                # >= WORKERS or high WORKERS serializes at ~10.
CONFIRM_AGGRESSIVE  = False    # no interactive prompt on a server
PRINT_EVERY         = 20       # progress line every N guests processed
DRY_RUN             = False

# Server-side ceilings so one job cannot hold the farm lock indefinitely.
MAX_TARGET_HEARTS = 300
MAX_WORKERS = 60
MAX_STREAM_CONCURRENCY = 6

# WORKERS/STREAM_CONCURRENCY are module globals the worker threads read at call
# time, so run_heart_job has to assign them. Keep the originals: without these,
# a job that raises the values would leave them raised, and the next job that
# passes nothing would silently inherit another user's concurrency — including
# the ban-risk lever.
DEFAULT_WORKERS = WORKERS
DEFAULT_STREAM_CONCURRENCY = STREAM_CONCURRENCY
...
def _pooled_session(pool_size):
    """A requests.Session whose urllib3 connection pool is sized to the
    concurrency we actually use. The default pool_maxsize is 10, so with
    WORKERS in the hundreds most login/initMember calls would otherwise fight
    over 10 connections (churning new ones) -- sizing the pool removes that
    hidden serialization. This is a pure perf change with no extra risk."""
    s = requests.Session()
    n = max(int(pool_size or 0), 10)
    adapter = requests.adapters.HTTPAdapter(pool_connections=n, pool_maxsize=n, max_retries=0)
    s.mount("https://", adapter)
    s.mount("http://", adapter)
    return s


_HTTP_POOL_N = HTTP_POOL_SIZE or WORKERS
_HTTP = _pooled_session(_HTTP_POOL_N)
# separate pooled session for guest creation / guest re-login (goes via proxy)
...
# ---- shared POOL of gRPC channels for ALL guests ----------------------------
# Every guest RPC authenticates via per-call metadata (Guest.md()), so channels
# can be shared across guests -- no need for one channel per guest (that was a
# TCP+TLS handshake per guest, up to ~300 per session). The safe version shares
# ONE channel; the fast version keeps a small POOL and hands them out round-
# robin, so a single HTTP/2 connection's concurrent-stream cap doesn't throttle
# the producer when WORKERS is high. Channels are thread-safe and multiplex, so
# this is pure connection headroom -- no behavior/concurrency change per call.
_GUEST_CHANNELS = []
_GUEST_CHANNEL_LOCK = threading.Lock()
_GUEST_RR = itertools.count()


def _guest_channel():
    chans = _GUEST_CHANNELS
    if not chans:
        with _GUEST_CHANNEL_LOCK:
            if not _GUEST_CHANNELS:
                creds = grpc.ssl_channel_credentials()
                _GUEST_CHANNELS.extend(
                    grpc.secure_channel(GRPC_ENDPOINT, creds)
                    for _ in range(max(1, GUEST_CHANNEL_POOL)))
        chans = _GUEST_CHANNELS
    return chans[next(_GUEST_RR) % len(chans)]


def _close_guest_channel():
    with _GUEST_CHANNEL_LOCK:
        for ch in _GUEST_CHANNELS:
            ch.close()
        _GUEST_CHANNELS.clear()
...
def run_heart_job(
    ...
    """Farm hearts for one account. One web token buys one call of this.

    Returns {"ok", "hearts", "target", "error"}; ok is False when nothing was
    collected, which is what tells the caller to refund.
    """
    global MAIN_EMAIL, MAIN_PASSWORD, TARGET_HEARTS, PROXY_URL, _PROXIES
    global WORKERS, STREAM_CONCURRENCY, _HTTP_POOL_N, _HTTP, _GUEST_HTTP

    if not email or not password:
        return {"ok": False, "error": "missing_credentials", "hearts": 0}

    MAIN_EMAIL = email
    MAIN_PASSWORD = password
    TARGET_HEARTS = _clamp(target_hearts, 100, 1, MAX_TARGET_HEARTS)
    WORKERS = _clamp(workers, DEFAULT_WORKERS, 1, MAX_WORKERS)
    STREAM_CONCURRENCY = _clamp(
        stream_concurrency, DEFAULT_STREAM_CONCURRENCY, 1, MAX_STREAM_CONCURRENCY
    )

    PROXY_URL = (proxy_url or "").strip()
    _PROXIES = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None

    # The pooled sessions were sized at import time from the old WORKERS value;
    # rebuild them so the pool actually matches this job's concurrency.
    _HTTP_POOL_N = HTTP_POOL_SIZE or WORKERS
    _HTTP = _pooled_session(_HTTP_POOL_N)
    _GUEST_HTTP = _pooled_session(_HTTP_POOL_N)
    ...
    main_acct = None
    try:
        print("heart: target=%d workers=%d concurrency=%d proxy=%s"
              % (TARGET_HEARTS, WORKERS, STREAM_CONCURRENCY, "yes" if PROXY_URL else "no"))
        ...
        return {
            ...
            "ok": total > 0,
            "hearts": total,
            "target": TARGET_HEARTS,
            "error": None if total > 0 else "no_hearts_collected",
        }
    except HeartError as exc:
        return {"mode": "heart", "ok": False, "hearts": 0, "error": exc.code, "detail": exc.detail}
    except SystemExit as exc:
        msg = str(exc)
        code = "login_failed" if "เข้าสู่ระบบไม่สำเร็จ" in msg or "LOGIN" in msg.upper() else "heart_error"
        return {"mode": "heart", "ok": False, "hearts": 0, "error": code, "detail": msg[:300]}
    except Exception as exc:
        msg = str(exc)
        code = "login_failed" if "เข้าสู่ระบบไม่สำเร็จ" in msg else "heart_error"
        return {"mode": "heart", "ok": False, "hearts": 0, "error": code, "detail": msg[:300]}
    finally:
...
def peek_friend_room(email, password, log_cb=None):
    """How many guest slots are free right now (300 minus real friends)."""
    import builtins
    old_print = builtins.print
    ...
    try:
        ...
        lf = main_acct.unary("service.api.FriendAPI", "ListFriends", {"common_req": {}})
        friends = len(lf.get("friends") or [])
        return {
            "ok": True,
            "friends": friends,
            "room": max(0, GAME_FRIEND_CAP - friends),
            "friend_cap": GAME_FRIEND_CAP,
            "max_target": MAX_TARGET_HEARTS,
        }
    except Exception as exc:
        msg = str(exc)
        code = "login_failed" if "เข้าสู่ระบบไม่สำเร็จ" in msg else "peek_failed"
        return {"ok": False, "error": code, "detail": msg[:300]}
    finally:
        builtins.print = old_print
        try:
            if main_acct is not None:
